import type { Translations } from "./types";

export const uk: Translations = {
  app_title: "SignStamp",
  open_pdf: "Відкрити PDF",
  pages_singular: "сторінка",
  pages_plural: "сторінок",
  zoom_out: "Зменшити",
  zoom_in: "Збільшити",
  tool_pan: "Перемістити",
  tool_text: "Текст",
  tool_date: "Дата",
  tool_check: "Хрестик",
  tool_ellipse: "Коло",
  tool_line: "Закреслення",
  tool_arrow: "Стрілка",
  tool_highlight: "Підсвітка",
  tool_sign: "Підпис",
  tool_paraph: "Парафа",
  undo: "Скасувати",
  theme_light: "Світла тема",
  theme_dark: "Темна тема",
  font_label: "Шрифт",
  size_label: "Розмір",
  bold: "Жирний",
  underline: "Підкреслення",
  strike: "Закреслення",
  font_sans: "Sans",
  font_serif: "Serif",
  font_mono: "Mono",
  stroke_label: "Товщина",
  ink_label: "Колір чорнила",
  color_black: "Чорний",
  color_blue: "Синій",
  color_red: "Червоний",
  color_green: "Зелений",
  import_signature: "Імпортувати підпис",
  draw_signature: "Намалювати підпис",
  draw_signature_title: "Намалювати підпис",
  draw_signature_hint: "Використай трекпад або мишу, щоб намалювати підпис.",
  draw_paraph: "Намалювати парафу",
  draw_paraph_title: "Намалювати парафу",
  draw_paraph_hint: "Використай трекпад або мишу, щоб намалювати парафу.",
  paraph_empty: "Спочатку намалюй парафу.",
  signature_clear: "Очистити",
  signature_cancel: "Скасувати",
  signature_save: "Зберегти",
  signature_empty: "Спочатку намалюй підпис.",
  clear_all: "Очистити все",
  delete: "Видалити",
  snippets_title: "Швидкі тексти",
  snippets_placeholder: "Додати текст для повторного використання",
  snippets_add: "Додати",
  snippets_empty: "Додай тексти, які часто використовуєш.",
  snippets_use: "Перетягніть, щоб вставити",
  snippets_remove: "Видалити текст",
  export_pdf: "Експорт PDF",
  print_pdf: "Друк",
  print_failed: "Не вдалося надрукувати PDF.",
  profile: "Профіль",
  profile_hint: "Заповни один раз — використовуй у кожній формі.",
  profile_add_key: "Додати власне поле",
  profile_add_key_placeholder: "Назва поля",
  remove_profile_entry: "Видалити це поле",
  autofill: "Авто-заповнення",
  autofill_title: "Перегляд авто-заповнення",
  autofill_text_fields: "текстове(і) поле(я) буде заповнено",
  autofill_signatures: "підпис(и) буде проставлено",
  autofill_paraph: "парафу додано на кожній сторінці",
  autofill_unmatched: "поле(я) не можна заповнити",
  autofill_apply: "Застосувати",
  autofill_no_matches: "Поля знайдені, але жодне не співпало — перевір профіль.",
  autofill_flat_pdf: "У цьому PDF немає полів форми — це плоский документ. Використай ручні інструменти.",
  autofill_paraph_medium: "Виявлена потенційна зона парафи з низькою впевненістю — розмісти її вручну за потреби.",
  signatures: "Підписи",
  remove_signature: "Видалити підпис",
  signatures_hint: "Імпортуйте PNG (краще) або JPG.",
  paraphs: "Парафи",
  remove_paraph: "Видалити парафу",
  paraphs_hint: "Імпортовані парафи з'являються на кожній сторінці.",
  import_paraph: "Імпортувати парафу",
  label_tool: "Інструмент",
  label_objects: "Обʼєкти",
  hint_line1: "Текст/Дата: клік — додати, подвійний клік — редагувати.",
  hint_line2: "Хрестик: клік — поставити. Підпис: виберіть і клікніть.",
  hint_line3: "Перетягування для переміщення. Ручка — змінити розмір.",
  empty_hint: "Заповнити та підписати PDF",
  empty_action: "Відкрити PDF",
  text_placeholder: "Текст...",
  text_prompt: "Текст:",
  edit_title: "Подвійний клік для редагування",
  export_success: "PDF експортовано до:\n{path}",
  export_failed: "Експорт не вдався під час створення PDF. Дивіться консоль.",
  export_tauri_failed: "Експорт Tauri не вдався. Буде спроба прямого завантаження.",
  no_pdf: "PDF не завантажено",
  file_label: "Файл: {name}",
  export_note: "Експорт = плаский PDF (текст + зображення вбудовано)"
};
